...
void DownloadWidget::on_pushButton_CertFile_clicked()
{
    ...
    if(!file_name.isEmpty())
    {
        ...
        {
            ui_->lineEdit_SecurityFile->setText(file_name);
        }
        else
        {
            ui_->lineEdit_SecurityFile->setText("");
        }
    }
}
...
void DownloadWidget::LoadLastScatterFile()
{
    ...
    if(!scatterFile_historyList_.empty())
    {
        ...
        if(last_dir.length() > 0)
        {
            ...
            path = QDir::toNativeSeparators((scatterFile_historyList_.at(0)));
...
{
    ui_->label_authFile->setVisible(show);
    ui_->lineEdit_authFilePath->setVisible(show);
    ui_->pushButton_authFile->setVisible(show);
    ui_->label_Security->setVisible(show);
    ui_->lineEdit_SecurityFile->setVisible(show);
...
void DownloadWidget::UpdateImageList(std::list<ImageInfo> &image_list)
{
    ...
    if(stricmp(info.version, "v1.1.1") > 0 && stricmp(info.storage, "EMMC") == 0)
        showRegion = true;
    ...
    main_window_->UpdateReadbackList(strcmp(info.storage, "EMMC") == 0 ? false : true);
    ...
    QSharedPointer<ConsoleMode::GeneralSetting> &gen_setting)
{
    ...
    arg->cert_file=ui_->lineEdit_SecurityFile->text().toUtf8().constData();
...
{
    if(ui_->lineEdit_SecurityFile->text().isEmpty())
    {
