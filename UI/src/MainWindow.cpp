...
void MainWindow::SetUI()
{
    ...
    if(ToolInfo::IsCustomerVer())
        ui->menuExport_Console_Mode_XML->menuAction()->setVisible(false);
...
void MainWindow::setChipInfo(ChipInfoWidget *widget, const DA_REPORT_T *p_da_report)
{
   ...
   QString chip_name = p_da_report->m_bbchip_name;
   QString chip_hw_ver;
...
void MainWindow::InitShortcuts()
{
    ...
    msecurity_shortcut = new QShortcut(this);
    msecurity_shortcut->setKey(Qt::CTRL + Qt::ALT + Qt::Key_S);
    connect(msecurity_shortcut, SIGNAL(activated()), this, SLOT(toggleEnableSecurityMode()));
    ...
    mDevTest_shortcut->setKey(Qt::CTRL + Qt::ALT + Qt::Key_T);
...
void MainWindow::toggleEnableSecurityMode()
{
    if(lockui_on)
        return;

    msecurity_on = !msecurity_on;
...
void MainWindow::on_actionShow_Welcome_triggered()
{
    if(show_welcome)
    {
        ...
        tab_widgets.remove(welcome_widget);
