    ...
    m_workerThread(NULL)
{
    ...
    ui->pushButton_browser->setVisible(false);
...
void OptionDialog::InitSettings()
{
    ...
    case DHNDL_FLAG_CHKSUM_LEVEL_NONE:
        ui->checkBox_emmcchksum->setCheckState(Qt::Unchecked);
        ui->checkBox_usbchksum->setCheckState(Qt::Unchecked);
        break;
    ...
    case DHNDL_FLAG_CHKSUM_LEVEL_ALL:
        ui->checkBox_emmcchksum->setCheckState(Qt::Checked);
        ui->checkBox_usbchksum->setCheckState(Qt::Checked);
        break;
    default:
...
void OptionDialog::LoadGeneralSettings()
{
    ...
    if(cleanLogIndex != -1)
        ...
        ui->comboBox__LogDate->setCurrentIndex(ui->comboBox__LogDate->count() - 1);
}
...
void OptionDialog::ResetShortKey(int row)
{
    ...
    if (it == bind_vector.end() )// Binding not used
    {
        ...
        settings.setArrayIndex(bind_vector.size());
        settings.endArray();
...
void OptionDialog::on_pushButton_apply_clicked()
{
    return;
    ...
    QString path_ = Utils::GetCurrentFileName(ui->comboBox__LogDate->currentIndex());
    QDate current = QDate::fromString(path_.remove("SP_FT_Dump_"), "MM-dd-yyyy");
