...
static const QString kDefaultRegionStr="EMMC_USER";
...
platform_changed_(false)
{
   ...
   read_region_map_["EMMC_BOOT_1"] =  EMMC_PART_BOOT1;
   read_region_map_["EMMC_BOOT_2"] = EMMC_PART_BOOT2;
   read_region_map_["EMMC_RPMB"] = EMMC_PART_RPMB;
   read_region_map_["EMMC_USER"] = EMMC_PART_USER;
...
void ReadBackWidget::UpdateReadBackItemsByScatter()
{
   ...
   for(;it!=image_info_list.end();++it)
   {
      ...
      //set region
      region_str = QString(QString::fromLocal8Bit(it->region.c_str()));
...
{    
      ...
      item = new QTableWidgetItem(kDefaultRegionStr);
      item->setTextAlignment(Qt::AlignVCenter | Qt::AlignHCenter);
      ui_->tableWidget->setItem(row, ColumnRegion, item);
...
ReadbackItem ReadBackWidget::GetRBItem(int row)
{
   ...
   EMMC_Part_E region = ParseRegionID(
...
EMMC_Part_E ReadBackWidget::ParseRegionID(const QString &region_str)
{
   EMMC_Part_E ret = EMMC_PART_UNKNOWN;

   std::map<QString,EMMC_Part_E>::const_iterator it = read_region_map_.find(region_str);
   if(it != read_region_map_.end())
   {
      ret = it->second;
   }
   else
   {
      LOG("Unknown region: %s",region_str.constData()->toAscii());
...
void ReadBackWidget::on_tableWidget_cellDoubleClicked(int row, int column)
{
   ...
   if(!file_name.isEmpty())
   {
      ...
      EMMC_Part_E region_id = ParseRegionID(
         ui_->tableWidget->item(row, ColumnRegion)->text());
      ...
      if(cnf == QDialog::Accepted)
      {
         ...
         EMMC_Part_E region_id = addr_dialog_->region_id();
