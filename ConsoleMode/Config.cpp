...
Config::Config(const CommandLineArguments& args)
{
    ...
    if (!config_file.empty())
    {
        ...
        LoadFile(config_file, args.OnlyOutput());
    }
    else
    {
...
void Config::LoadFile(const std::string &file_name, bool efuse_read_only)
{
    ...
    m_pclCommandSetting = QSharedPointer<CommandSetting>(new CommandSetting(cmds_node, efuse_read_only));
}
...
bool Config::fgIsCommboFmt(QSharedPointer<AppCore>& app,const APKey& key) const
{
    ...
    if(stricmp(version, "v1.1.1") > 0 && storage == HW_STORAGE_EMMC )
        return true;
