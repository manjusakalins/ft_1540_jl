    ...
    const HW_StorageType_E type, const QSharedPointer<Connection> & conn)
{
    ...
    {
        return QSharedPointer<APCore::AutoFormatArg>(new NANDFormatArg(GET_DL_HANDLE_T(key_)));
    }
    else if(HW_STORAGE_EMMC == type)
    {
        if(part_id >= EMMC_PART_UNKNOWN && part_id < EMMC_PART_END && !is_normal_fmt_)
            return QSharedPointer<APCore::AutoFormatArg>(new EMMCFormatArg(conn, true, part_id, false));

        return QSharedPointer<APCore::AutoFormatArg>(new EMMCFormatArg(conn));
    }
    else if(HW_STORAGE_SDMMC == type)
    {
        return QSharedPointer<APCore::AutoFormatArg>(new SDMMCAutoFormatArg(conn));
    }
    Q_ASSERT(0);
